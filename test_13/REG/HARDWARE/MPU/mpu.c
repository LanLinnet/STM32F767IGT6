...
{
	MPU->CTRL=(1<<2)|(1<<0);	//打开背景region，使能MPU
	SCB->SHCSR|=1<<16;			//使能MemManage
}

//将nbytes转换为2为底的指数（用于设置REGIONSIZE）
//返回指数值
...
//设置某个区域的MPU保护（设置RNR、RBAR、RASR三个寄存器）
//baseaddr:MPU保护区域的基址(首地址)
//size:MPU保护区域的大小(必须是32的倍数,单位为字节)
//rnum:MPU保护区编号,范围:0~7,最大支持8个保护区域

//ap:访问权限,访问关系如下:
//0,无访问（特权&用户都不可访问）		1,仅支持特权读写访问
//2,禁止用户写访问（特权可读写访问）	3,全访问（特权&用户都可访问）
//4,无法预测(禁止设置为4!!!)			5,仅支持特权读访问
//6,只读（特权&用户都不可以写）
...
u8 MPU_Set_Protection(u32 baseaddr, u32 size, u32 rnum, u8 ap, u8 sen, u8 cen, u8 ben)
{
	u32 tempreg=0;
	u8 rnr=0;
	if((size%32)||size==0) return 1;		//大小不是32的倍数或size为0，说明参数错误
	rnr=MPU_Convert_Bytes_To_POT(size)-1;	//将大小转换为以2为底的指数值
	
	MPU_Disable();		//设置之前，先禁止MPU保护
	MPU->RNR=rnum;		//设置保护区域（区域编号寄存器）
	MPU->RBAR=baseaddr;		//设置基址（基地址寄存器）
	
	tempreg|=0<<28;				//允许指令访问（允许读取指令）
	tempreg|=((u32) ap)<<24;	//设置访问权限
	tempreg|=0<<19;				//设置类型拓展域为level0    
	tempreg|=((u32) sen)<<18;	//是否允许公用
	tempreg|=((u32) cen)<<17;	//是否允许cache
	tempreg|=((u32) ben)<<16;	//是否允许缓冲
	tempreg|=0<<8;				//禁止子区域
	tempreg|=rnr<<1;			//设置保护区域大小
	tempreg|=1<<0;				//使能该保护区域
	MPU->RASR=tempreg;			//设置RSAR寄存器
