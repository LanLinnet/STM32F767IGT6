...
u32 *ltdc_framebuf[2];					//LTDC LCD帧缓存数组指针,必须指向对应大小的内存区域,LEDC两个层所以数组大小为2
...
{
	if(sw==1)LTDC->GCR|=1<<0;			//打开LTDC 使能
...
void LTDC_Layer_Switch(u8 layerx,u8 sw)
{
	...
	LTDC->SRCR|=1<<0;							//重新加载配置  LTDC Shadow Reload Configuration Register  Bit 0 IMR: Immediate Reload
}
